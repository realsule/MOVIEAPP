...
/* =====================================================
   🎬 Movie Box SPA
   - Fetches shows from your Render-hosted JSON server
   - Features: search, filter by genre, seat booking
   - Persists bookings using localStorage
   ===================================================== */

// ----- Config -----
const BASE_URL = "https://movie-api-1-ihlq.onrender.com"; // ✅ your Render API
const MAX_SHOWS = 12;
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      occupiedSeatsMap: Object.fromEntries(
        Object.entries(appState.occupiedSeatsMap).map(([k, s]) => [
          k,
          Array.from(s),
        ])
      ),
      ...
      return `
        <article class="card" data-id="${show.id}">
          <div class="poster">
            ${
              img
                ? `<img src="${img}" alt="${escapeHtml(show.name)} poster">`
                : `<div style="padding:8px;color:var(--muted);font-size:12px">No Image</div>`
            }
          </div>
          <div class="info">
            <h3 class="title">${escapeHtml(show.name)}</h3>
            <div class="meta">${escapeHtml(genres)} • ${escapeHtml(
        String(show.rating?.average || "N/A")
      )}</div>
...
function renderGenreOptions() {
  ...
  genreFilter.innerHTML = [
    ...
    ...arr.map((g) => `<option value="${g}">${g}</option>`),
  ].join("");
}
...
function renderBookings() {
  ...
  bookList.innerHTML = arr
    ...
    <div class="book-item">
      ...
      <div style="color:var(--muted);font-size:13px">Seats: ${b.seats.join(
        ", "
      )} • Date: ${b.date}</div>
    </div>`
    )
...
function renderSeatsForShow(showId) {
  seatsGrid.innerHTML = "";
  if (!appState.occupiedSeatsMap[showId])
    appState.occupiedSeatsMap[showId] = new Set();
...
function openModalWithShow(show) {
  ...
  modalMeta.textContent = `${(show.genres || []).join(", ")} • Runtime: ${
    show.runtime || "N/A"
  } min`;
  modalSummary.textContent = shortText(
    show.summary || "No description available",
    100
  );

  trailerFrame.src =
    trailerMap[show.name] || "https://www.youtube.com/embed/default";
...
function handleConfirmBooking() {
  ...
  if (appState.selectedSeats.size === 0)
    return alert("Please select seats before booking.");
  ...
  appState.selectedSeats.forEach((sid) =>
    occupied.add(String(seatIndexFromId(sid)))
  );
  ...
  alert(
    `✅ Success — booked ${seatsArr.length} seat(s) for ${appState.selectedShow.name}.`
  );
  closeModalHandler();
  renderBookings();
}
...
searchEl.addEventListener("input", (ev) => {
  ...
  appState.filtered = appState.shows.filter((show) =>
    show.name.toLowerCase().includes(q)
  );
  const genre = genreFilter.value;
  if (genre && genre !== "all") {
    appState.filtered = appState.filtered.filter((s) =>
...
genreFilter.addEventListener("change", (ev) => {
  ...
  if (genre === "all") {
    appState.filtered = appState.shows.filter((s) =>
      s.name.toLowerCase().includes(q)
    );
  } else {
    appState.filtered = appState.shows.filter(
      (s) =>
        s.name.toLowerCase().includes(q) && (s.genres || []).includes(genre)
    );
  }
  setGrid(appState.filtered);
});
...
async function loadAndRenderShows() {
  try {
    ...
    const resp = await fetch(`${BASE_URL}/shows`);
    ...
    appState.shows.forEach((s) =>
      (s.genres || []).forEach((g) => appState.genres.add(g))
    );
